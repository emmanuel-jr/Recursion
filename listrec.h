...

{
	...
	if (p->dataItem == 'b')
	{
		ListNode<DT>* newNodePtr = new ListNode<DT>('a', p);
			head = newNodePtr;

	}

	aBeforebSub(head->next);*/


	ListNode<DT>* p = new ListNode<DT>('\0', head);
	head = p;
	aBeforebSub(head);
...

{
	...
	ListNode<DT>* currPtr = p;
	if (currPtr->next != nullptr)
	{
		if ('b' == currPtr->next->dataItem)
		{
			ListNode<DT>* newNodePtr = new ListNode<DT>('a', currPtr->next);
			currPtr->next = newNodePtr;
			currPtr = currPtr->next;
		}

		aBeforebSub(currPtr->next);
	}
}